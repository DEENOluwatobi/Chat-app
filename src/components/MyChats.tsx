...
interface MyChatsProps {
  ...
  setSelectedChat: (value: boolean) => void;
  setDetailPic: (value: boolean) => void;
}

const MyChats: React.FC<MyChatsProps> = ({ setChatBox, setSelectedChat, setDetailPic }) => {
    ...
    <div className='h-full w-[410px] bg-secondary border-r border-slate-700 relative'>
      ...
      <div className='flex w-full h-[53px] py-[7px] px-3'>
        ...
        <button className={`text-icon h-7 w-7 flex p-1 ml-2 mt-1 justify-center items-center ${filter ? "bg-line rounded-full full" : ""}`} onClick={() => setFilter(!filter)}>
          <svg viewBox="0 0 24 24" width="20" height="20" preserveAspectRatio="xMidYMid meet" className=""><path fill="currentColor" d="M10 18.1h4v-2h-4v2zm-7-12v2h18v-2H3zm3 7h12v-2H6v2z"></path></svg>
        </button>
      </div>

      <div className='w-full h-[83%] overflow-y-scroll'>
        {filter ?
          <UnreadedChat name={''} isGroupChat={false}/>
          :
          user.map((value : any) => {
          return (
